...
Zero dependencies. Single HTML files. Work offline. Deployed builds ship with the inline JavaScript obfuscated; the readable source lives in `public/`.
...
├── build.js                   # Build script: public/ -> dist/ with inline JS obfuscated
├── dist/                      # Generated deploy output (gitignored) — what wrangler serves
├── archive/                   # Old snapshots kept for reference, never deployed
├── deploy.sh                  # Builds dist/ then deploys all three Workers
...
The files in `public/` are the readable source and remain fully usable when opened directly. The deployed sites serve obfuscated copies generated by `npm run build` (see [Build / obfuscation](#build--obfuscation)).
