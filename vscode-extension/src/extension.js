...
function activate(context) {
    ...
    const output = vscode.window.createOutputChannel('CodeGraph Apex/LWC Dependency Trace Parents');
    context.subscriptions.push(output);

    // Full CodeGraph Apex/LWC Dependency experience — architecture map, Analyze Flow, Trace
        ...
        await vscode.window.withProgress(
            {
                ...
                title: 'CodeGraph Apex/LWC Dependency: scanning workspace files',
                cancellable: false,
            },
    ...
    async function traceFrom(componentName, filePath) {
        ...
        if (!root) {
            vscode.window.showWarningMessage('CodeGraph Apex/LWC Dependency: open a project folder/workspace first.');
            return;
        }
        try {
            await ensureIndex(root, false);
        } catch (e) {
            vscode.window.showErrorMessage(`CodeGraph Apex/LWC Dependency Trace Parents failed: ${e.message || e}`);
            return;
        }
    ...
    context.subscriptions.push(
        vscode.commands.registerCommand('apexflowTraceParents.traceFromFile', async (uri) => {
            ...
            if (!target) {
                vscode.window.showWarningMessage('CodeGraph Apex/LWC Dependency: open or right-click a .js/.html file first.');
                return;
            }
    ...
    context.subscriptions.push(
        vscode.commands.registerCommand('apexflowTraceParents.traceSelection', async () => {
            ...
            if (!raw) {
                vscode.window.showWarningMessage('CodeGraph Apex/LWC Dependency: highlight a component name first, then right-click.');
                return;
            }
            const name = normalizeSelectedComponentName(raw);
            if (!name) {
                vscode.window.showWarningMessage(`CodeGraph Apex/LWC Dependency: couldn't find a component name in "${raw}".`);
                return;
            }
    ...
    context.subscriptions.push(
        vscode.commands.registerCommand('apexflowTraceParents.refresh', async () => {
            if (!provider.rootNode) {
                vscode.window.showInformationMessage('CodeGraph Apex/LWC Dependency: nothing traced yet — run "Trace Parents…" first.');
                return;
            }
            ...
            try {
                ...
                vscode.window.showErrorMessage(`CodeGraph Apex/LWC Dependency rescan failed: ${e.message || e}`);
                return;
            }
            provider.refresh();
        })
